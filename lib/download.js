...
async function dlAction() {
    ...
    const jsonConfig = await fse.readJson(cfgPath)  // Spinner 初始设置  
    ...
    try {    // 下载模板后解压    
        ...
        dlSpinner.text = chalk.red(`Download template failed. ${err}`)    // 终止等待动画并显示 X 标志    dlSpinner.fail()    process.exit()
    }  // 下载成功时提示  
